...
if __name__ == '__main__':
    ...
    for subpath in os.listdir(path0):
        if r"split" + str(args.split) in subpath:
            ...
            if len(matching_files1) > 0 or len(matching_files2) > 0:
                #os.rename(matching_files1[0], path0 + subpath + '/Spin_2305_converge.npy')
                #os.rename(matching_files2[0], path0 + subpath + '/Spin_unet_converge.npy')
                sample_count += 1
            else:
            ...
            if len(indexmm[0]) > 0 and len(indexun[0]) > 0:
                ...
                cw_vortex_2305, cw_antivtx_2305, \
                ccw_vortex_2305, ccw_antivtx_2305 = analyze_winding(spin_2305, model)
                
                vortex_unet, antivtx_unet, \
                pp_vortex_unet, pp_antivtx_unet, \
                np_vortex_unet, np_antivtx_unet, \
                cw_vortex_unet, cw_antivtx_unet, \
                ccw_vortex_unet, ccw_antivtx_unet = analyze_winding(spin_unet, model)
                ...
                if vortex_2305 == vortex_unet and antivtx_2305 == antivtx_unet:
                    ...
                    if pp_vortex_2305  == pp_vortex_unet   and \
                        ...
                        cw_antivtx_2305 == cw_antivtx_unet  and \
                        ccw_vortex_2305 == ccw_vortex_unet  and \
                        ccw_antivtx_2305== ccw_antivtx_unet:
                        exact_match += 1
                        ...
                        print("Clockwise antivtx : {}   {}".format(cw_antivtx_2305,  cw_antivtx_unet))
                        print("CounterCW vortex  : {}   {}".format(ccw_vortex_2305,  ccw_vortex_unet))
                        print("CounterCW antivtx : {}   {}".format(ccw_antivtx_2305, ccw_antivtx_unet))
                else:
                    print("\n"+subpath)
                    print("-------            2305  Unet")
                    print("Vortex    cores   : {}   {}".format(vortex_2305, vortex_unet))
    ...
    Summary-- Statistic_samples [{}/{}] Unet k{} mmPre_core {} size {} split {} 
              ...
              Statistic_precision : single_domain   {:.4f} ; single_vortex  {:.4f}
                                    single_antivtx  {:.4f} ; multiple_vtx   {:.4f}\n
    ...
    '''.format(converge_sample_count, sample_count, args.krn, args.pre_core, args.w, args.split, 
               ...
               1 - abs(none_unet - none_2305)/none_2305                               if none_2305 !=0 else np.nan,
               1 - abs(single_vortex_unet - single_vortex_2305)/single_vortex_2305    if single_vortex_2305 !=0 else np.nan,
               1 - abs(single_antivtx_unet - single_antivtx_2305)/single_antivtx_2305 if single_antivtx_2305 !=0 else np.nan,
               1 - abs(multi_unet - multi_2305)/multi_2305                            if multi_2305 !=0 else np.nan
