...
def analyze_winding(spin, model):
    ...
    curl = (model > 0) * (( spin_xp[:,:,1] - spin_xm[:,:,0] )/2
                       -  ( spin_yp[:,:,1] - spin_ym[:,:,0] )/2 )
    
    cw_abs  = np.abs(winding_density[curl<0]).sum()
    ccw_abs = np.abs(winding_density[curl>0]).sum()
    cw_sum  = winding_density[curl<0].sum()
    ccw_sum = winding_density[curl>0].sum()
    
    cw_vortices = (cw_abs + cw_sum) /2
    cw_antivortices = (cw_abs - cw_sum) /2
    ccw_vortices = (ccw_abs + ccw_sum) /2
    ccw_antivortices = (ccw_abs - ccw_sum) /2
    ...
    return np.around(vortex_cores), np.around(antivortex_cores), \
           ...
           np.around(cw_vortices),  np.around(cw_antivortices), \
           np.around(ccw_vortices), np.around(ccw_antivortices)
